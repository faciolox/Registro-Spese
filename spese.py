...
def get_spesa_mensile(json_spese: [Spesa], mese = None, anno = datetime.now().year):
    ...
    for v in json_spese:
        if inizio <= datetime.strptime(v["Orario"], "%d/%m/%Y %H:%M:%S") < fine:
            sp = Spesa(v["Importo"], v["Descrizione"], datetime.strptime(v["Orario"], "%d/%m/%Y %H:%M:%S"))
            out.append(sp)
            totale += v["Importo"]
...
def add_addebito_cc(json_spese:[Spesa],spesa,mensilità):
    ...
    for i in range(0, mensilità):
        ...
        if i + month > 12:
            m -= 12
            for j in json_spese:
                if datetime.strptime(j["Orario"], "%d/%m/%Y %H:%M:%S").month + 1== m and j["Descrizione"]== "Addebito c/c" and datetime.strptime(j["Orario"], "%d/%m/%Y %H:%M:%S").year == year:
                    j["Importo"] += spesa
                    found = True
                    break
            if not found:
